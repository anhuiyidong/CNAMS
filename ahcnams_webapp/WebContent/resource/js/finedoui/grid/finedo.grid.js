	...
	$.fn.grid = function(options){
        	...
        	for(; rowindex < datalength; rowindex++){
    			...
    			for(var colindex = 0; colindex < columns.length; colindex++){
					...
					if(finedo.fn.istrue(columns[colindex].canhide)){
						td.addClass("finedo-page-canhide");
					}
    				tr.append(td);
    			}
    			//增加扩展列
    			...
    			tbody.append(tr);
            	if($finedogrid.needhide()){
    				$(".finedo-page-canhide").hide();
    			}
    			//判断是否需要继续加载
        ...
        this.execquery = function(){
			...
			if($finedogrid.getascitems().length > 0){
				...
				loadurl += "asc="+$finedogrid.getascitems().join();
			}
			if($finedogrid.getdescitems().length > 0){
				loadurl += loadurl.indexOf("?") == -1 ? "?" : "&";
				loadurl += "desc="+$finedogrid.getdescitems().join();
			}
	        ...
	        $.ajax({
	            ...
	            error: function(jqXHR, textStatus, errorThrown){
	            	if(jqXHR.readyState != 4)
	            		return;
	            	finedo.message.error("服务器请求异常:"+errorThrown);
	            },
	            complete: function(e){
	            	finedo.message.hidewaiting();
	            }
		...
		this.initPagination = this.initpagination = function(pageoption){
			...
			'<label><span class="finedo-color finedo-page-canshow" id="'+$tableid+'curtotalpage"></span>共<span class="finedo-color" id="'+$tableid+'total">0</span>条</label></div>' +
			...
			if($finedogrid.needhide(true)){
				$(".finedo-page-canhide").hide();
				$(".finedo-page-canshow").show();
			}else{
				$(".finedo-page-canshow").hide();
			}
		...
		this.initHead = this.inithead = function(columns){
			...
			for(var headindex = 0; headindex < columns.length; headindex++){
				...
				if(finedo.fn.istrue(columns[headindex].canhide)){
					$tditem.addClass("finedo-page-canhide");
				}
