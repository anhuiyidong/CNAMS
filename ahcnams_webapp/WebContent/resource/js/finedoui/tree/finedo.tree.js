;(function($, undefined) {
	...
	$.fn.tree = function(options){
		 ...
		 * 加载树节点
		 */
		this.loadNodes = this.loadnodes = function(parentNode, data){
			var rootopen = true;
			...
			for(var i = 0; i < data.length; i++){
				...
				if(!rootopen && finedo.fn.istrue(data[i].open))
					rootopen = true;
				...
				if(finedo.fn.isnon(data[i].children)){
					...
					switchNode = $('<span id="'+$treeid+'_'+data[i].id+'_ico" class="'+(finedo.fn.isnotnon(data[i].icon) ? iconemptycls : (finedo.fn.istrue($finedotree.getoption("async")) ? iconclosecls : icondocucls))+'"></span>');
					aNode = $('<a id="'+$treeid+'_'+data[i].id+'_a" title="'+data[i].name+'">\
							<span id="'+$treeid+'_'+data[i].id+'_span">'+data[i].name+'</span></a>');
				}else{
					switchNode = $('<span id="'+$treeid+'_'+data[i].id+'_ico" class="'+(finedo.fn.isnotnon(data[i].icon) ? iconemptycls : (rootopen ? iconopencls: iconclosecls))+'"></span>');
					aNode = $('<a id="'+$treeid+'_'+data[i].id+'_a" title="'+data[i].name+'">\
							<span id="'+$treeid+'_'+data[i].id+'_span">'+data[i].name+'</span></a>');
				}
				//判断节点是否自定义了显示图标
				if(finedo.fn.isnotnon(data[i].icon)){
					$(switchNode).css('background-image','url('+data[i].icon+')');
				}
				node.append(switchNode);
				if($finedotree.data('options').selecttype == 'multi'){
					checkNode = $('<span id="'+$treeid+'_'+data[i].id+'_check" class="'+fulluncheckcls+'"></span>');
					if(finedo.fn.istrue(data[i].checked)){
				...
				if(finedo.fn.istrue($finedotree.getoption('async')) || finedo.fn.isNotNon(data[i].children)){
		...
		this.load = function(reqdata){
			...
			$.ajax({
	            ...
	            success: function(data){
					...
					$finedotree.loadNodes(finedo.fn.isnon(reqdata) ? $finedotree : $('#'+$treeid+'_'+reqdata.id+'_ul'), objdata);
	            }
	        });
		};
