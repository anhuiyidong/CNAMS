 ...
 */
//文件查询url
var file_queryurl = ctx + "/finedo/file/query";
//文件上传url
var file_uploadurl = ctx + "/finedo/file/upload";
//文件删除url
var file_deleteurl = ctx + "/finedo/file/delete";
//单个文件下载url
var file_downloadurl = ctx + "/finedo/file/download";
//多个文件下载url
var file_downloadmultiurl = ctx + "/finedo/file/downloadmulti";
...
;(function($, undefined) {
	...
	$.fn.file = function(options){
        ...
        this.uploadCheck = this.uploadcheck = function(){
        	$("#"+$finedofile.attr('id')+"uploadfrm").append($('#'+$finedofile.attr('id')+'filename'));
        	//$('#'+$finedofile.attr('id')+'filename').hide();
        ...
        this.showFiles = this.showfiles = function(data){
        	...
        	for(var fileindex = 0; fileindex < data.length; fileindex++){
            	...
            	//预览按钮
            	var fileopdiv = $('<div class="finedo-upload-fileitem-name">');
            	
            	if($finedofile.canpreview(data[fileindex])){
	            	var previewitem = $('<div class="finedo-upload-op-icon">预览</div>');
	            	fileopdiv.append(previewitem);
	            	previewitem.click(data[fileindex], function(event){$finedofile.preview(event.data);});
            	}
            	
            	var downloaditem = $('<div class="finedo-upload-op-icon finedo-upload-op-download">下载</div>');
            	fileopdiv.append(downloaditem);
            	if(finedo.fn.istrue($finedofile.data('options').editable)){
                	var deleteitem = $('<div class="finedo-upload-op-icon finedo-upload-op-del">删除</div>');
        ...
        this.preview = function(){
        	...
        	for(var i = 0; i < filelist.length; i++){
        		...
        		if($finedofile.canpreview(filedata)){
        			previewhtml+='<img alt="'+filedata.filename+'" data-original="'+ctx+'/finedo/file/download?fileid='+filedata.fileid+'" src="'+ctx+'/finedo/file/downloadthumbnail?fileid='+filedata.fileid+'"></img>';
        		}
        	}
        	if(previewhtml == '<div>')
        		return;
        	previewhtml += '</div>';
        	$(previewhtml).viewer({"url":"data-original"}).viewer("show");
        };
        ...
        this.init = function(){
        	...
        	if(finedo.fn.isnon(options.queryurl))
        		$finedofile.setqueryurl(file_queryurl);
        	if(finedo.fn.isnon(options.uploadurl))
        		$finedofile.setuploadurl(file_uploadurl);
        	if(finedo.fn.isnon(options.deleteurl))
        		$finedofile.setdeleteurl(file_deleteurl);
        	if(finedo.fn.isnon(options.downloadurl))
        		$finedofile.setdownloadurl(file_downloadurl);
        	if(finedo.fn.isnon(options.downloadmultiurl))
        		$finedofile.setdownloadmultiurl(file_downloadmultiurl);
