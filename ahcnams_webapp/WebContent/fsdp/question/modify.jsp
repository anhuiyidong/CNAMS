...
<div class="add-common-head">
    ...
    <div id="common_add_div" >
    	<div class="finedo-nav-title">基本信息</div>
	   	<ul class="finedo-ul">
    	<form method="post" action="${ctx }/finedo/question/modify" id="ajaxForm" name="ajaxForm">
    		<input type="hidden" id="questionid" name="questionid" value="${sysquestion.questionid }">
    		<input type="hidden" id="attachment" name="attachment" value="${sysquestion.attachment }">
    		<input type="hidden" id="status" name="status" value="${sysquestion.status }">
			<li>
				<span class="finedo-label-title"><font color=red>*</font>问题标题</span>
				<fsdp:text id="title" style="width:600px;" value="${sysquestion.title }"/>
			</li>

           <li>
           		<span class="finedo-label-title"><font color=red>*</font>问题上报人</span>
           		<input type="hidden" id="initiator" name="initiator" value="${sysquestion.initiator}">
           		<fsdp:text id="initiatorname" value="${sysquestion.initiatorname }" onclick="doSelectUser()"/>
           	</li>

           <li>
           		<span class="finedo-label-title"><font color=red>*</font>问题发生时间</span>
           		<fsdp:date id="happentime" datefmt="yyyy-MM-dd HH:mm:ss" value="${sysquestion.happentime }"/>
           	</li>

           	<li>
           		<span class="finedo-label-title"><font color=red>*</font>问题描述</span>
           		<fsdp:textarea id="questiondesc">${sysquestion.questiondesc }</fsdp:textarea>
           	</li>
    	</form>
        	<li>
        		<span class="finedo-label-title">附件</span>
				<input type="text" id="uploaddiv" name="uploaddiv" valueid="attachment" value="${sysquestion.attachment }" multiupload="true" maxfilenum="5">
			</li>
        </ul>
        <ul>
