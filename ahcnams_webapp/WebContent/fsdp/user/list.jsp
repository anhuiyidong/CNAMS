...
function formatOperation(row){
	...
	var operation = '<a href="javascript:showModifyDialog(\'' + row.user.userid + '\');">[编辑]</a>&nbsp;';
	operation += '<a href="javascript:finedo.action.doDelete(\'datagrid\',\'${ctx}/finedo/user/deleteUser\',\''+row.user.userid+'\')">[删除]</a>&nbsp;';
	
	 var user='${LOGINDOMAIN_KEY.sysuser.userid}';
	 if(user=="superuser"){
		 operation += '<a href="javascript:doPassword(\'' + row.user.userid + '\', \''+row.user.usercode+'\', \''+row.user.phoneno+'\');">[密码重置]</a>&nbsp;';
	 }
		

	if("锁定" == row.user.state){
		operation += '<a href="javascript:doLockUser(\'unlock\',\''+row.user.userid+'\')">[解锁]</a>&nbsp;';
	}else{
		operation += '<a href="javascript:doLockUser(\'lock\',\''+row.user.userid+'\')">[锁定]</a>&nbsp;';
...
function formatUserid(row){
	return '<a href="javascript:showDetailDialog(\'' + row.user.userid + '\');">'+row.user.usercode+'</a>&nbsp;';
}

// 单元格重载
function formatPersonname(row){
	return '<a href="javascript:showDetailDialog(\'' + row.user.userid + '\');">'+row.user.personname+'</a>&nbsp;';
}
