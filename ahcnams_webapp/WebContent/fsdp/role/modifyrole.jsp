...
$(document).ready(function() {
	...
	//初始化角色权限树
	$.getJSON('${ctx}/finedo/permission/getpermissiontree', {'roleid':'${param.roleid}'}, function(jsondata){
		if(jsondata.fail){
	...
	//查询角色基本信息
	$.getJSON('${ctx}/finedo/role/queryId', {'roleid':'${param.roleid}'}, function(jsondata){
		if(jsondata.fail){
			finedo.message.error(jsondata.resultdesc);
			return;
		}
		...
		$("#dutydesc").val(roledata.dutydesc);
		finedo.commonui.setRadioVal('roletype', roledata.roletype);
		finedo.commonui.setRadioVal('rolelvl', roledata.rolelvl);
		finedo.commonui.setRadioVal('state', roledata.state);
	});
});
...
<div>
    ...
    <form method="post" action="${ctx }/finedo/role/modifyRole" id="ajaxForm" name="ajaxForm">
    	...
    	<div class="finedo-nav-title">基本信息</div>
	   	<table class="finedo-table">
		    <tr>
		        ...
		        <td>
		        	<fsdp:radio datasource="岗位角色类型" id="roletype" selectedvalue="标准岗位角色"></fsdp:radio>
		        </td>
		    ...
		    <tr>
		        ...
		        <td>
		        	<fsdp:radio datasource="岗位角色级别" id="rolelvl"></fsdp:radio>
		        </td>
		        <td class="finedo-label-title"><font color=red>*</font>组织节点：</td>
		        <td>
		        	<fsdp:radio datasource="岗位角色状态" id="state"></fsdp:radio>
		        </td>
		    </tr>
		    <tr>
