...
<div class="head">
    ...
    <div class="head-nav">
        <ul>
	        ...
	        <c:forEach var="userpermission" items="${sessionScope.LOGINDOMAIN_KEY.userpermissionlist}" >
		        <c:if test="${userpermission.parentnodeid eq '0' and userpermission.isnavigation eq '是' and nodecount < 8 }">
		       		...
		       		<script>
			       		...
			       		window.onload=function(){
			       			showsubmenu("${userpermission.nodeid }");
			       		}
		       		</script>
		       		</c:if>
		       		
					<li><a href="#" onclick="chooseonenode(this); showsubmenu('${userpermission.nodeid}')" id="${userpermission.nodeid}">${userpermission.nodename }</a></li>
				</c:if>
	        </c:forEach>
        </ul>
    </div>
    ...
    <div class="head-operate" onMouseOut="hideviewdiv()" >
        ...
        <div class="head-menu" id="head-menu" onMouseOver="showviewdiv()" style=" display:none;" >
            <c:forEach var="userpermission" items="${sessionScope.LOGINDOMAIN_KEY.userpermissionlist}" >
            	<c:if test="${userpermission.parentnodeid eq '0' and userpermission.isnavigation eq '是'}">
            		<a href="#" onclick="chooseviewnode(this); showsubmenu('${userpermission.nodeid}')" class="head-a-link">${fsdp:substr(userpermission.nodename, 4)}</a>
            	</c:if>
            </c:forEach>
        </div>
    </div>
...
<div class="menu" id="menu" style=" display:block; height:100%; ">
    ...
    <c:forEach var="userpermission1" items="${sessionScope.LOGINDOMAIN_KEY.userpermissionlist}" >
    	<c:if test="${userpermission1.parentnodeid eq '0' and userpermission1.isnavigation eq '是'}">
    	<div class="menu-submenu" id="submenu_${userpermission1.nodeid }" style="display:none">
		    <div class="menu-submenu-name menu-font13">${userpermission1.nodename }</div>
		    	
	    	<div class="menu-submenu-con">
			    <ul>
		    	<c:forEach var="userpermission2" items="${sessionScope.LOGINDOMAIN_KEY.userpermissionlist}" >
			        <c:if test="${userpermission2.parentnodeid eq userpermission1.nodeid and userpermission2.isnavigation eq '是'}">
			        	<li><a class="menu-submenu-a" onClick="submenu(this)" href="${ctx }${userpermission2.permissionentry }" target="mainFrame"><span class="menu-icon"></span>${userpermission2.nodename }</a></li>
			        </c:if>
			    </c:forEach>
			    </ul>
		    </div>
