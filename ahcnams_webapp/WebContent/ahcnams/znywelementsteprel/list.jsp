...
function formatOperation(row){
	...
	operation += '<a href="#" onclick="finedo.action.doDelete(\'datagrid\',\'${ctx}/finedo/znywelementsteprel/delete\',\'' + row.relid + '\')">[删除]</a>&nbsp;';
	return operation;
}
...
function doQuery() {
	...
	var param = {relid: $('#relid').val(), eletid: $('#eletid').val(), stepid: $('#stepid').val()};
...
function showModifyDialog(pkeyid) {
	finedo.dialog.show({
		...
		height:550,
		'title':'修改信息',
		'url':'${ctx}/finedo/znywelementsteprel/modifypage?relid=' + pkeyid
	});
}
...
function showDetailDialog(pkeyid) {
	finedo.dialog.show({
		...
		height:550,
		'title':'详情信息',
		'url':'${ctx}/finedo/znywelementsteprel/detail?relid=' + pkeyid
	});
}
...
function doExport() {
	...
	$("#downiframe").attr('src', '${ctx }/finedo/znywelementsteprel/exportexcel' + (param ? '?' + $.param(param) : ''));
}
/************** 增、删、改函数定义****************/
...
<div style="width:100%;">
	...
	<div class="query-title">
		<!-- 标题 -->
    	<div class="query-title-name">网元和模板步骤对应关系管理 </div>
    	
    	<div class="query-boxbig">
	    	<input type="button" class="query-btn-nextstep" onclick="showAddDialog();" value="新建网元和模板步骤对应关系">
    ...
    <!-- 表格栏  -->
    <fsdp:grid id="datagrid" url="${ctx }/finedo/znywelementsteprel/query" selecttype="multi" expand="doExpandRow">
		<fsdp:field code="relid" name="关系id" width="100" formatter="formatPkey" order="true"></fsdp:field>
		<fsdp:field code="eletid" name="网元id" width="100"></fsdp:field>
